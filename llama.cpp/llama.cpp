...
struct token_grammar {
    ...
    bool accept_str(const char *s, bool special_tokens) {
        if(strlen(s) == 0) {
            return accept('\0');
        }
        if (special_tokens) {
...
void llama_grammar_accept_token(struct llama_context * ctx, llama_token id, void* filter_ptr, bool special_tokens) {
    if(!filter_ptr) {
        return;
    }
    ...
    if (!filter->accept_str(as_str, special_tokens))
    {
